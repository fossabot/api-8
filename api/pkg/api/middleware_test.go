...
func TestMiddlewareReturnUnauthorized(t *testing.T) {
	var h Handler = func(ctx context.Context, req Request) Response {
	...
	resp := m(h)(testhelper.NewContext(), NewDummyRequest())
...
func TestMiddlewareRunOrder(t *testing.T) {
	...
	m(h)(testhelper.NewContext(), NewDummyRequest())
	if !run {
...
func TestWithMiddleware(t *testing.T) {
	...
	h(testhelper.NewContext(), NewDummyRequest())
	expected := []string{
		"m3-before",
		"m2-before",
		"m1-before",
